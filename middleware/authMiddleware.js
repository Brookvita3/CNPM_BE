...
function authMiddleware(req, res, next) {
  ...
  try {
    ...
    req.user = { userId: decoded.id }; // Lưu thông tin user từ token vào req
    next();
  } catch (err) {
...
function generateAccessToken(userId) {
  return jwt.sign(userId, process.env.SECRET_KEY_ACCESS, {
...
function generateRefreshToken(userId) {
  return jwt.sign(userId, process.env.SECRET_KEY_REFRESH, {
